...
flarum_forum_app__WEBPACK_IMPORTED_MODULE_0___default().initializers.add('shebaoting/ai-assistant', function () {
  (0,flarum_common_extend__WEBPACK_IMPORTED_MODULE_1__.extend)((flarum_forum_components_Post__WEBPACK_IMPORTED_MODULE_2___default().prototype), 'view', function (vdom) {
    var aiUserId = 993; // 替换为您的 AI 用户 ID

    // 正确访问用户 ID，使用 user() 方法并调用 id()
    var author = this.attrs.post.user();
    var authorId = author ? author.id() : null;
    if (authorId == aiUserId) {
      console.log('你好');
      // 检查是否已经插入提示，避免重复插入
      if (!vdom.children.some(function (child) {
        return child.attrs && child.attrs.className == 'ai-reply-label';
      })) {
        // 在帖子内容下方添加一个带有样式的标签
        vdom.children.push(m('div.TagLabel.colored.text-contrast--light.TagLabel--child', '此回复由AI生成'));
      }
    }
  });
});
